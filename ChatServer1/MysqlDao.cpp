...
int MysqlDao::RegUser(const std::string& name, const std::string& email, const std::string& pwd)
{
	...
	try {
		...
		// 准备调用存储过程
		//prepareStatement创建一个预处理语句对象(sql::PreparedStatement)
		//CALL是调用存储过程的关键字
		unique_ptr < sql::PreparedStatement > stmt(con->_con->prepareStatement("CALL reg_user(?,?,?,@result)"));
		// 设置输入参数
		stmt->setString(1, name);//第一个？
		stmt->setString(2, email);//第二个？
		stmt->setString(3, pwd);//第三个？
...
//查询“别人给我发的好友申请”，并且按申请 id 从小到大分页返回
