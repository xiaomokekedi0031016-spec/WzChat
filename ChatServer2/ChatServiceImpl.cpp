...
{
	//cout << "8888888888888888888888888888";
	//查找用户是否在本服务器
...
Status ChatServiceImpl::NotifyAuthFriend(ServerContext* context, const AuthFriendReq* request, AuthFriendRsp* reply) {
	...
	auto session = UserMgr::GetInstance()->GetSession(touid);//对端
	Defer defer([request, reply]() {
		reply->set_error(ErrorCodes::Success);
	...
	Json::Value rtvalue;
	rtvalue["error"] = ErrorCodes::Success;
	rtvalue["fromuid"] = request->fromuid();//申请认证的uid
	rtvalue["touid"] = request->touid();//被申请认证的uid
